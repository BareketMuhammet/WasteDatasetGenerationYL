# Blender 3.6.x
# Juice Carton Crushing (vacuum + flatten + fold)
# ------------------------------------------------
# - Clears scene
# - Imports juice carton FBX (aligned to Z)
# - Adds a strong cloth "vacuum" collapse
# - Adds a large crease band
# - Flattens thickness and shortens height with lattices
# - Bends carton along a vertical curve (S-shaped fold)
# - Exports the crushed carton as FBX
...
def import_juice_carton(filepath):
    """
    Import FBX and try to pick the juice-carton mesh.
    """
    log(f"Importing juice carton FBX from: {filepath}")
    NAME_HINTS = ["carton", "juice", "box"]
    ...
    for hint in NAME_HINTS:
        for o in mesh_candidates:
            if hint.lower() in o.name.lower():
                log(f"Using carton object by hint '{hint}': {o.name}")
                return o

    if not mesh_candidates:
        raise RuntimeError("No mesh objects found in imported FBX.")

    carton_obj = mesh_candidates[0]
    log(f"Using carton object: {carton_obj.name}")
    return carton_obj
...
def create_pin_group_top_bottom(obj, lower_bound=0.1, upper_bound=0.9):
    ...
    """
    PIN_GROUP_NAME = "Carton_Pin_TopBottom"
...
def setup_cloth_sim(obj, pin_group, pressure_force, shrink_factor):
    log("Adding cloth simulation to carton...")

    CLOTH_MOD_NAME = "Carton_Cloth"
    ...
    # Softer carton-like material – fairly wrinkly
    ...
    if eff is not None:
        eff.gravity = 0.0  # we mainly rely on pressure
...
def create_vertical_bend_curve(obj, bend_offset_factor_x=0.0, bend_offset_factor_y=0.25):
    """
    Simple vertical Bezier curve used for overall carton bending.
    """
    log("Adding vertical (Z-axis) Bezier curve for carton bending...")

    CURVE_NAME = "Carton_Bend_Curve"
...
def add_curve_modifier_after_cloth(obj, curve_obj):
    ...
    CURVE_MOD_NAME = "Carton_Curve_Deform"
...
def rotate_object_vertices_z(obj, angle_degrees):
    """
    Rotate vertices around Z (randomizing artwork orientation).
    """
...
def create_carton_lattice(obj,
                          lattice_name="CartonLattice",
...
def flatten_carton_y_lattice(obj, scale_factor=0.15, keep_center=True, points_v=4):
    """
    Flatten carton thickness (Y axis) – like being squashed onto its side.
    """
    lat_obj, lat_mod = create_carton_lattice(
        obj,
        lattice_name="CartonYLattice",
...
def squash_carton_z_lattice(obj, scale_factor=0.5, anchor_mode='BOTTOM', points_w=6):
    """
    Shorten carton height (Z) using a lattice – gives a more crushed look.
    """
    lat_obj, lat_mod = create_carton_lattice(
        obj,
        lattice_name="CartonZLattice",
...
def main():
    # --- EDIT THESE PATHS ---
    fbx_filepath = r"/home/tem/Waste-Dataset-Generation/fbx_standard/beverage_carton/carton1.fbx"
    export_path = r"/home/tem/Waste-Dataset-Generation/res_fbx_objects/carton_crushed.fbx"

    clear_scene()

    carton_obj = import_juice_carton(fbx_filepath)

    bpy.context.view_layer.objects.active = carton_obj
    carton_obj.select_set(True)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    carton_obj.select_set(False)

    # Randomize label orientation
    angle = np.random.uniform(0, 360)
    rotate_object_vertices_z(carton_obj, angle)
    ...
    press_band_inward(carton_obj, band_half_size, scale_factor_y, center_offset)
    ...
    pin_group = create_pin_group_top_bottom(carton_obj, lower_bound, upper_bound)

    pressure_force = np.random.uniform(-800, -300)   # strong vacuum
    shrink_factor = np.random.uniform(-0.4, -0.01)    # contract carton surface
    setup_cloth_sim(carton_obj, pin_group, pressure_force, shrink_factor)

    # 3) Bend carton with a curve
    ...
    if np.random.rand() < two_peaks_vertical_squash_probability:
        ...
        curve_obj = create_vertical_bend_curve_2_peaks(carton_obj, height_factor=height_factor, end_offset_factor_y=end_offset_factor_y, points_per_arc=64, smooth=True)
    else:
        ...
        curve_obj = create_vertical_bend_curve(carton_obj, bend_offset_factor_x, bend_offset_factor_y)
    add_curve_modifier_after_cloth(carton_obj, curve_obj)

    flatten_probability = 0.5
    if np.random.rand() < flatten_probability:
        # 4) Flatten thickness (Y) – carton becomes very thin like in reference
        y_scale = np.random.uniform(0.1, 0.5)
        flatten_carton_y_lattice(carton_obj, scale_factor=y_scale, keep_center=True, points_v=4)

    # 5) Shorten height (Z) a bit for a “crumpled and shoved down” look
    z_scale = np.random.uniform(0.35, 1)
    squash_carton_z_lattice(carton_obj, scale_factor=z_scale, anchor_mode='BOTTOM', points_w=6)
    ...
    set_origin_to_evaluated_surface_com(carton_obj)
    ...
    log(f"Exported crushed carton to: {export_path}")
