...
#     # Purge orphan data recursively (cleans depsgraph leftovers)
#     try:
#         bpy.ops.outliner.orphans_purge(do_recursive=True)
#     except Exception:
#         pass

#     # Optional: disable undo during batch runs to avoid undo stack growth
#     try:
...
def deform_export_soda_can(source_fbx_path, export_fbx_path, vertical_squash_probability=0.3, two_peaks_vertical_squash_probability = 0.3):
    """
    ...
    can_obj = import_soda_can(source_fbx_path)
    ...
    log(f"Exported: {export_fbx_path}")
