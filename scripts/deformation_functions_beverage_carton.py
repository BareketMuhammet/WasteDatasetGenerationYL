...
def import_beverage_carton(filepath):
    """
...
def make_name(prefix, idx):
    return f"{prefix}_{idx:06d}.fbx"

def deform_export_beverage_carton(source_fbx_path, export_fbx_path, flatten_probability = 0.5, press_band_inward_twice_probability = 0.3, two_peaks_vertical_squash_probability = 0.3):
    """
    Runs a single generation pass using your existing pipeline,
    but parameterized by input/output file paths.
    """

    clear_scene()

    carton_obj = import_beverage_carton(source_fbx_path)
    ...
    log(f"Exported to: {export_fbx_path}")
