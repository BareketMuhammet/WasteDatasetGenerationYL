...
def build_asset_pools(
    ...
    for category in categories:
        ...
        if not assets:
            print(f"No FBX assets found for category '{category.name}'.")
            continue
...
def choose_asset(
    ...
    if not available_pools:
        ...
        if unclassified_choice is not None:
            asset_path, apply_transparency = unclassified_choice
            return asset_path, None, apply_transparency
        raise RuntimeError("No FBX objects available to instantiate.")
    ...
    for _ in range(object_count):
        ...
        if category is None:
            ...
            if category.name.lower() == "pet":
                blue_color = random.random() < 0.25
                if blue_color:
                    print("blue pet")
                    color_r = random.uniform(0.6, 0.68)
                    color_g = random.uniform(0.88, 0.96)
                    color_b = random.uniform(0.98, 1.0)
                    color_override = (color_r, color_g, color_b, 1.0)
                else:
                    print("white pet")
                    color_p = random.uniform(0.95, 1.0)
                    color_override = (color_p, color_p, color_p, 1.0)

            obj.pass_index = pass_index
            apply_transparency(obj, category.transparency, color_override=color_override)
